...
function start_vm {
  ...
  startup_script="
    ...
    RUNNER_ALLOW_RUNASROOT=1 cd /actions-runner && ls && ./config.cmd --url https://github.com/${GITHUB_REPOSITORY} --token ${RUNNER_TOKEN} --labels ${VM_ID} --unattended ${ephemeral_flag} --disableupdate --runasservice && \\
